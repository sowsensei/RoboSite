...
коммита в папку сайта. Отдельной заливки медиа не требуется. Единственное,
что при этом собирается, — шапка и футер (см. раздел «Шапка и футер» ниже).
...
Шапка и футер лежат в `partials/header.html` и `partials/footer.html` — это
единственное место, где их правят. В страницах вместо них стоят плейсхолдеры:
...
Подставляются они **дважды, и это специально**:

1. **При деплое** — хук запускает `deploy/build-partials.sh`, и тот заменяет
   плейсхолдеры содержимым партиалов прямо в разложенной копии, до выкладки в
   `/var/www/robo`. То есть на сайт уезжает HTML, где шапка и футер уже внутри.
2. **В браузере** — `js/main.js` подставляет их, если плейсхолдер в странице
   всё-таки остался. Это страховка: если сборка не отработает, сайт продолжит
   работать как раньше, просто на JS.

Зачем понадобился п.1, если п.2 и так работал: JS-вариант вклеивает футер уже
после загрузки, поэтому в исходном HTML его нет — а поисковик смотрит именно
исходный HTML. Для нас это критично из-за ссылки **«Дата-центр itsoft»** в
футере: она обязана быть индексируемой по условиям бесплатного хостинга (тот
же блок есть в футере школьного сайта). Заодно роботу становятся видны ссылки
меню в шапке, а страница не «прыгает» без шапки первые миллисекунды.

Что из этого следует:

- **Правишь шапку/футер — правишь один файл**, запускать ничего не надо: push
  сам всё вклеит.
- Пути внутри партиалов — **только абсолютные** (`/assets/…`, `/lessons/…`),
  иначе они сломаются на вложенных страницах.
- **Папку `partials/` из выкладки убирать нельзя** — её грузит JS-страховка.
- Новую страницу заводи копированием существующей, вместе с плейсхолдерами.
  Если плейсхолдера нет, скрипт напишет предупреждение в вывод пуша, но деплой
  не уронит.
- Посмотреть, что получится, локально: `bash deploy/build-partials.sh .` —
  результат коммитить не нужно (в репозитории страницы живут с плейсхолдерами).

> ⚠️ Хук на сервере — не тот же файл, что в репозитории. После правок
> `deploy/post-receive` его нужно один раз переложить на сервер (см. п. 2
> разовой настройки), иначе сборка выполняться не будет.
